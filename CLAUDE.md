...
2. **Does every task rely only on concepts already taught earlier** — earlier in this same document's section order, or in an already-completed phase? If a task genuinely needs a preview of later material, say so explicitly and give the minimal syntax inline, rather than silently assuming knowledge the reader doesn't have. (Hit this with Ex 3.5's `read_numbers`, which needed file I/O — Section 7 material — while sitting in Section 3.)
